...
class PositionController {
	...
	public void setImageSize(int width, int height) {
		...
		float ratio = Math.min((float) mImageW / width, (float) mImageH
				/ height);
		...
		if (position != null) {
			...
			mCurrentX = Math.round((mViewW / 2f - position.x) / scale)
					+ mImageW / 2;
			mCurrentY = Math.round((mViewH / 2f - position.y) / scale)
					+ mImageH / 2;
			mCurrentScale = scale;
		...
		if(mViewer.mScaleMin==0){
			mViewer.mScaleMin = mCurrentScale;
		}
		if(mViewer.mScaleMin!=0 && mViewer.mScreenScale==0){
			mViewer.mScreenScale = mCurrentScale;
		}
		//bill 20150126 end
		mViewer.setPosition(mCurrentX, mCurrentY, mCurrentScale);
	}
	...
	public void beginScale(float focusX, float focusY) {
		mInScale = true;
		mFocusBitmapX = Math.round(mCurrentX + (focusX - mViewW / 2f)
				/ mCurrentScale);
		mFocusBitmapY = Math.round(mCurrentY + (focusY - mViewH / 2f)
				/ mCurrentScale);
	}
	...
	public void startSlideInAnimation(int direction) {
		int fromX = (direction == PhotoView.TRANS_SLIDE_IN_LEFT) ? mImageW / 2
				: 3 * mImageW / 2;
	...
	private void scrollBy(float dx, float dy, int type) {
		startAnimation(getTargetX() + Math.round(dx / mCurrentScale),
				getTargetY() + Math.round(dy / mCurrentScale), mCurrentScale,
				type);
	}
	...
	public boolean fling(float velocityX, float velocityY) {
		...
		// return false;
		// }
		if (mImageW * mCurrentScale <= mViewW + 1
				&& mImageH * mCurrentScale <= mViewH + 1) {
			return false;
		}
		// lianglei end
		...
		mScroller.fling(mCurrentX, mCurrentY,
				...
				Math.round(-velocityY / mCurrentScale), mBoundLeft,
				mBoundRight, mBoundTop, mBoundBottom);
	...
	public boolean advanceAnimation() {
		...
		// lianglei begin 设置横屏和竖屏图片的切换动画一致
		 mViewer.setPosition(mCurrentX, mCurrentY, mCurrentScale);
		 //bill add begin 20150126 设置壁纸所见即所得
		 mViewer.pmCurrentX = mCurrentX;
		 mViewer.pmCurrentY = mCurrentY;
		 mViewer.pmCurrentScale = mCurrentScale;
		//bill add end 20150126 设置壁纸所见即所得
//		if (mImageH < mImageW) {
//			mViewer.setPosition(mViewer.getHeight(),
//					mCurrentX * mViewer.getWidth() / mViewer.getHeight(),
//					mCurrentScale);
//		} else {
//			mViewer.setPosition(mCurrentX, mCurrentY, mCurrentScale);
//		}
