...
import com.ktouch.kdc.launcher4.app.CropImage;
...
public class MenuExecutor {
	...
	private static ProgressDialog showProgressDialog(Context context,
		...
		// lianglei begin 去掉删除的时候闪现的dialog
		// dialog.show();
		// lianglei end
		return dialog;
	}
	...
	public MenuExecutor(GalleryActivity activity,
	...
	private static void setMenuItemVisibility(Menu menu, int id,
			boolean visibility) {
	...
	public static void updateMenuOperation(Menu menu, int supported) {
		...
		setMenuItemVisibility(menu, R.id.action_crop, false);
		setMenuItemVisibility(menu, R.id.action_share, false);
		setMenuItemVisibility(menu, R.id.action_setas, false);
		setMenuItemVisibility(menu, R.id.action_show_on_map, false);
		setMenuItemVisibility(menu, R.id.action_edit, false);
		setMenuItemVisibility(menu, R.id.action_details, false);
		setMenuItemVisibility(menu, R.id.action_import, false);
		// setMenuItemVisibility(menu, R.id.action_setting, true);

		setMenuItemVisibility(menu, R.id.action_slideshow, false); // bill
																	// add
		...
		// setMenuItemVisibility(menu, R.id.action_show_on_map,
		// supportShowOnMap);
	...
	public boolean onMenuClicked(MenuItem menuItem, ProgressListener listener) {
		...
		case R.id.action_crop: {
			...
			Intent intent = new Intent(CropImage.ACTION_CROP).setDataAndType(
	...
	private boolean execute(DataManager manager, JobContext jc, int cmd,
		...
		case R.id.action_show_on_map: {
			...
			if (GalleryUtils.isValidLocation(latlng[0], latlng[1])) {
				GalleryUtils.showOnMap((Context) mActivity, latlng[0],
						latlng[1]);
			}
			break;
		}
		...
		case R.id.action_edit: {
			...
			try {
				...
				Toast.makeText(
						activity,
						activity.getString(R.string.imagepreviewer_activity_not_found),
