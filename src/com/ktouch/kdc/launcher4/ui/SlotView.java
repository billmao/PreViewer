...
import com.ktouch.kdc.launcher4.anim.Animation;
...
import com.ktouch.kdc.launcher4.util.LinkedNode;
...
public class SlotView extends GLView {
    ...
    public static  float scail = 0.2f;
    ...
    //bill add 
    public final static int TOP_HEIGHT=20;
    ...
    //bill add begin
    public static void setSlotViewWide(boolean wide){
    	WIDE = wide;
    }
    //bill add end
	...
	Log.v("billmao", "onTouchActivityDown "+factor);
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
            	Log.v("billmao", "onTouchActivityDown111111 "+factor);
            	factor = factor -scail ;
                mDownInScrolling = !mScroller.isFinished();
                mScroller.forceFinished();
                break;
            case MotionEvent.ACTION_MOVE:
            	Log.v("billmao", "onTouchActivityDown2222222 "+factor);
            	factor = factor +scail ;
    ...
    public static class Layout {
        ...
        private void initLayoutParameters(
            ...
            //bill modify 滑动到底部时，最后一个行底部文字显示不全
            //mContentLength = count * majorUnitSize + (count - 1) * mSlotGap;
            mContentLength = count * majorUnitSize + (count - 1) * mSlotGap+TOP_HEIGHT;
            //bill modify 滑动到底部时，最后一个行底部文字显示不全
        ...
        private void initLayoutParameters() {
            ...
            if (mSpec.slotWidth != -1) {
                ...
                //增加SlotView与左右两边的距离
                mSlotHeight = Math.max(1, (mWidth - (rows - 1) * mSlotGap) / rows);
                //bill modify end
        ...
        public int getSlotIndexByPosition(float x, float y) {
        	//bill modify WIDE改为了！WIDE begin
            ...
            if (WIDE && columnIdx >= mUnitCount) {//bill 此处!WIDE改为了WIDE
                return INDEX_NONE;
            }
  ...
  //bill modify WIDE改为了！WIDE end
    private class MyGestureListener implements
        ...
        public boolean onFling(MotionEvent e1,
            ...
            float velocity = !WIDE ? velocityX : velocityY; //bill wide改为！wide
        ...
        public boolean onScroll(MotionEvent e1,
            ...
            cancelDown();
            float distance = !WIDE ? distanceX : distanceY;//bill wide改为！wide
    ...
    public boolean setSlotCount(int slotCount) {
        ...
        updateScrollPosition(!WIDE ? mScrollX : mScrollY, true);//bill wide改为！wide
        return changed;
    }
