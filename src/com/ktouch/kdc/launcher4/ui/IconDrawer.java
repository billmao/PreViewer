...
public abstract class IconDrawer extends SelectionDrawer {
    ...
    public IconDrawer(Context context) {
        ...
        //bill add 1027 解决内存溢出问题，将图片压缩后图片显示变小问题，这些图片设置标记不进行压缩
        mLocalSetIcon.setInSamleFlag(false);
        mCameraIcon.setInSamleFlag(false);
        mPicasaIcon.setInSamleFlag(false);
        mMtpIcon.setInSamleFlag(false);
        
        mPanoramaBorder = new NinePatchTexture(context, R.drawable.ic_pan_thumb);
        mFramePressed = new NinePatchTexture(context, R.drawable.grid_imageviewer_pressed);
        mFrameSelected = new NinePatchTexture(context, R.drawable.grid_imageviewer_selected);
