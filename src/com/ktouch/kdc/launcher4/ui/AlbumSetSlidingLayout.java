...
//bill create
public class AlbumSetSlidingLayout extends ViewGroup {
     ...
     * mDockWidth in px
     */
    private static final float SLIDING_WIDTH = 190.0f;			
     ...
     * mVelocityUnits in px
     */
    private static final float DEF_SHADOW_WIDTH = 16.0f;		
	 ...
	 */
    public AlbumSetSlidingLayout(Context context, AttributeSet attrs) {
	 ...
	 */
    public AlbumSetSlidingLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		//CustomScrollView.fastScrollStatus== false 解决时间滑块滑动快速滑动有时滑动一段距离后就划不动了   20141210
		//mIsOpen 解决侧滑栏打开后在滑动listview禁止它滑动   20150114
        if (((action == MotionEvent.ACTION_MOVE)
      		  && (mTouchState != TOUCH_STATE_REST) && CustomScrollView.fastScrollStatus== false) || ((action == MotionEvent.ACTION_MOVE)
      		  && mIsOpen)
      		  ) { //bill 20141210
        	  //&& (mTouchState != TOUCH_STATE_REST){   
        	  return true;
        ...
        case MotionEvent.ACTION_MOVE:
                 ...
                 if (xDiff > mTouchSlop) { 
                      ...
                      if (Math.abs(mLastMotionY - y) / Math.abs(mLastMotionX - x) < 1 && 
                    		  (CustomScrollView.fastScrollStatus== false)) //bill 20141210
                               mTouchState = TOUCH_STATE_SCROLLING;
                }
                break;
