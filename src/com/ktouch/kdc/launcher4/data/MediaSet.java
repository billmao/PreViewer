...
import com.ktouch.kdc.launcher4.app.AlbumSetActivityDataAdapter;
...
public abstract class MediaSet extends MediaObject {
    ...
    //bill add begin 全部壁纸隐藏文件夹
    private AlbumFilterDatabase mDatabase;
    private ArrayList<String> name=new ArrayList<String>();
    private ArrayList<String> disFolderName=new ArrayList<String>();
    //bill add end
    ...
    public void getDatabaseInfo(Context context){
		...
		mDatabase = new AlbumFilterDatabase(context);
		mDatabase.openMyDataBase();
		cursor = mDatabase.queryAllHideFolder("Y");
		name.clear();
		...
		cursor.close();
		//bill add begin 20150115 解决从pc向手机内拷贝一个图片文件夹，图片在全部壁纸中显示的问题（实际不应显示）
		disCursor = mDatabase.queryAllHideFolder("N");
		disFolderName.clear();
		while (disCursor.moveToNext()) {
			disFolderName.add(disCursor.getString(cursor
					.getColumnIndex("AlbumName")));
		}
		disCursor.close();
		//bill add end 20150115 解决从pc向手机内拷贝一个图片文件夹，图片在全部壁纸中显示的问题（实际不应显示）
    ...
    protected int getIndexOf(Path path, ArrayList<MediaItem> list) {
    	//bill modify begin 全部壁纸删除到最后一张时崩溃
    	try{
    ...
    protected int enumerateTotalMediaItems(
        ...
        for (int i = 0; i < m; i++) {
            ...
            {
            	if(getSubMediaSet(i).getName().equals(name.get(j))){
            		existFlag = true;
            		break;
            	}
            }
            ...
            if(existFlag == false){
            	boolean flag = false;
            	for(int j =0;j<disFolderName.size();j++)
	            {
	            	if(getSubMediaSet(i).getName().equals(disFolderName.get(j))){
	            		flag = true;
	            		break;
	            	}
	            }
