...
import com.ktouch.kdc.launcher4.launcher2.FolderIcon.FolderRingAnimator;
...
public class CellLayout extends ViewGroup {
    ...
    int[] findNearestArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY,
        ...
        Log.v("songxj","findNearstArea spanX euqal is "+spanX);
        
        Log.v("songxj","findNearstArea spanY euqal is "+spanY);
        
        Log.v("songxj","findNearstArea minSpanX euqal is "+minSpanX);
        
        Log.v("songxj","findNearstArea minSpanY euqal is "+minSpanY);
        
        Log.v("songxj","findNearstArea mCellWidth euqal is "+mCellWidth + "   mCellHeight "+ mCellHeight);
        
        Log.v("songxj","findNearstArea mWidthGap euqal is "+mWidthGap + "   mHeightGap "+ mHeightGap);
        ...
        for (int y = 0; y < countY - (minSpanY - 1); y++) {
            ...
            for (int x = 0; x < countX - (minSpanX - 1); x++) {
                ...
                if (ignoreOccupied) {
                    ...
                    Log.v("songxj","hitMaxXhitMaxX +"+hitMaxX);
                    Log.v("songxj","hitMaxYhitMaxY +"+hitMaxY);
                    while (!(hitMaxX && hitMaxY)) {
                    	Log.v("songxj"," while (!(hitMaxX && hitMaxY)) QQQQQQQQQQQQQQQQQQQQQQQQQ");
                        if (incX && !hitMaxX) {
                ...
                for (Rect r : validRegions) {
                    if (r.contains(currentRect)) {
                        contained = true;
                        Log.v("songxj","Rect r:validRegions"+contained);
                        break;
                    }
                }
                validRegions.push(currentRect);
                Log.v("songxj","cellXY[] cellXY[0] "+cellXY[0]);
                
                Log.v("songxj","cellXY[] cellXY[1] "+cellXY[1]);
                
                Log.v("songxj","findNearstArea pixelX euqal is "+pixelX + "   pixelY "+ pixelY);
               // find nearest space modify for maobo
                ...
                if ((distance <= bestDistance && !contained) ||
                    ...
                    Log.v("maobo","!contained  +"+ contained );
                    bestXY[0] = x;
                    bestXY[1] = y;
                    if (resultSpan != null) {
                        resultSpan[0] = xSize;
