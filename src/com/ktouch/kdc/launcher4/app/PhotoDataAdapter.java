...
public class PhotoDataAdapter implements PhotoPage.Model {
    ...
    private static int DATA_CACHE_SIZE = 1;
    ...
    private MediaItem mData[] = new MediaItem[DATA_CACHE_SIZE];
    ...
    //bill begin 1217
    public interface NotifyChangeCollectStutas{
    	public void onChangeCollectStutas(Object object);
    }
    private static NotifyChangeCollectStutas collectStatus;
    //bill add end
    ...
    //bill add 1217 
    public static void setNotifyChangeCollectStutas(NotifyChangeCollectStutas status){
    ...
    private void updateCurrentIndex(int index) {
        ...
        //bill begin 1217
        collectStatus.onChangeCollectStutas(item.getDetails().getDetail(MediaDetails.INDEX_PATH).toString());
        //bill add end
    }
    ...
    private void updateImageRequests() {
        ...
        //bill add 1218 begin
        if(item!=null){
            if(Gallery.getGalleryIntance().dialogPhotoBarIsShowing()){
        ...
        public void run() {
            while (mActive) {
                ...
                synchronized (DataManager.LOCK) {
                    ...
                    //修改图片超过32张图片预览时有时出现的一直处于加载状态无法显示以及下面的缩略图显示排序不正确的问题
                    //bill modify begin 
                    if(isFirstLoad){
                    	isFirstLoad = false;
                    	DATA_CACHE_SIZE = info.size;
                    	mData = null;
