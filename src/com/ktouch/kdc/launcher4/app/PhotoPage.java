...
import com.ktouch.kdc.launcher4.data.MtpDevice;
import com.ktouch.kdc.launcher4.data.Path;
import com.ktouch.kdc.launcher4.picasasource.PicasaSource;
import com.ktouch.kdc.launcher4.ui.DetailsHelper;
import com.ktouch.kdc.launcher4.ui.FilmStripView;
import com.ktouch.kdc.launcher4.ui.GLCanvas;
import com.ktouch.kdc.launcher4.ui.GLView;
import com.ktouch.kdc.launcher4.ui.ImportCompleteListener;
...
import com.ktouch.kdc.launcher4.ui.PreviewImageView;
import com.ktouch.kdc.launcher4.ui.SelectionManager;
import com.ktouch.kdc.launcher4.ui.SynchronizedHandler;
import com.ktouch.kdc.launcher4.ui.UserInteractionListener;
import com.ktouch.kdc.launcher4.ui.DetailsHelper.CloseListener;
import com.ktouch.kdc.launcher4.ui.DetailsHelper.DetailsSource;
...
public class PhotoPage extends ActivityState implements
	...
	// private ActionBar mActionBar; //bill delete
	...
	// bill add begin
	public static PreviewImageView preview;
	...
	private void initFilmStripView() {
		...
		mFilmStripView.setVisibility(GLView.INVISIBLE);// lianglei 设置为初始不显示bar
		mShowBars = false;// lianglei 更新mShowBars

		// bill add begin
		if (mFilmStripView.getVisibility() == View.VISIBLE) {
			Log.e("scroll", "photo true" + this.toString());
			Gallery.getGalleryIntance().setPhotoPageMenuBarVisibility(true);
		} else {
			Log.e("scroll", "photo false" + this.toString());
	...
	public void onCreate(Bundle data, Bundle restoreState) {
		// mActionBar = ((Activity) mActivity).getActionBar(); //bill
		// delete
		...
		// bill add begin
		display = ((Activity) mActivity).getWindowManager().getDefaultDisplay();
		screenHeight = display.getHeight();
		screenWidth = display.getWidth();
		preview = new com.ktouch.kdc.launcher4.ui.PreviewImageView(
				mApplication.getAndroidContext(), R.drawable.nothing,
				screenWidth, screenHeight);
		mRootPane.addComponent(preview);
		// bill add end
		String setPathString = data.getString(KEY_MEDIA_SET_PATH);
		Path itemPath = Path.fromString(data.getString(KEY_MEDIA_ITEM_PATH));
		mPhotoViewPath = itemPath;
		if (setPathString != null) {
			...
			PhotoDataAdapter pda = new PhotoDataAdapter(mActivity, mPhotoView,
					mMediaSet, itemPath, mCurrentIndex);
			mModel = pda;
			mPhotoView.setModel(mModel);

			mPhotoView.setModelNew(mModel);
	...
	// bill delete actionbar begin
	// private void setTitle(String title) {
	// if (title == null) return;
	// boolean showTitle =
	// mActivity.getAndroidContext().getResources().getBoolean(
	...
	private void updateCurrentPhoto(MediaItem photo) {
		...
		// setTitle(photo.getName()); //bill delete
		mPhotoView
	...
	private void showBars() {
		...
		if (mFilmStripView != null) {
			// mFilmStripView.show(); //bill 删除预览界面中的浏览图 20150123
		}
		Gallery.getGalleryIntance().setPhotoPageMenuBarVisibility(true); // bill
																			// add
																			// begin
																			// �����˵�����ʾ
	}
	...
	private void hideBars() {
		...
		Gallery.getGalleryIntance().setPhotoPageMenuBarVisibility(false); // bill
																			// add
																			// begin
																			// ���ض����˵���
	}
	...
	protected boolean onItemSelected(MenuItem item) {
		...
		case R.id.action_import:
			mSelectionManager.deSelectAll();
			mSelectionManager.toggle(path);
			mMenuExecutor.onMenuClicked(item, new ImportCompleteListener(
					mActivity));
			return true;
		default:
			return false;
	...
	// lianglei begin
	public static void setImageview(int id) {
		preview.setImageId(id);
		// setAlphaAnim();//动画效果
	...
	public static void setAlphaAnim() {
